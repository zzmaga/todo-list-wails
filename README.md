...
### 🎨 Интерфейс
- 🌙 **Темная/светлая тема** - переключение тем одним кликом
- 📱 **Адаптивный дизайн** - корректное отображение на разных размерах окна
...
### Файловое хранение (по умолчанию)
Данные сохраняются в JSON файл в пользовательской директории. Никакой дополнительной настройки не требуется.

### PostgreSQL база данных
**✅ PostgreSQL уже настроен и работает!**

Приложение по умолчанию использует PostgreSQL базу данных. Если PostgreSQL недоступен, приложение автоматически переключится на файловое хранение.
...
#### Переключение на файловое хранение:
В файле `main.go` измените:
```go
Type: "file" // вместо "postgres"
```

Подробная документация: [DATABASE_SETUP.md](DATABASE_SETUP.md) | [Демонстрация PostgreSQL](demo_postgres.md)
